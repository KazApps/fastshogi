    ...
    void parse(const Args &args) {
        ...
        for (auto &[flag, params] : deferred) {
            try {
                ...
                auto err = fmt::format(R"(Error while reading option "{}" with value "{}")", flag, joinParams(params));
                auto msg = fmt::format("Reason: {}", e.what());
    ...
    parseFunc wrapHandler(const std::string &flag, Handler &&handler) {
        if constexpr (Style == ParamStyle::None) {
            ...
            return [flag, fn](const std::vector<std::string> &params, ArgumentData &data) {
                if (!params.empty()) {
                    throw fastshogi_exception("Option \"" + flag + "\" does not accept parameters.");
                }
                fn(data);
            };
            ...
            return [flag, fn](const std::vector<std::string> &params, ArgumentData &data) {
                if (params.size() != 1) {
                    throw fastshogi_exception("Option \"" + flag + "\" expects exactly one value.");
                }
            ...
            return [flag, fn](const std::vector<std::string> &params, ArgumentData &data) {
                if (params.empty()) {
                    throw fastshogi_exception("Option \"" + flag + "\" expects key=value parameters.");
                }
                ...
                for (const auto &param : params) {
                    ...
                    if (pos == std::string::npos || pos == 0 || pos + 1 == param.size()) {
                        throw fastshogi_exception("Option \"" + flag + "\" expects key=value pairs, got \"" + param +
                                                  "\".");
                    }
