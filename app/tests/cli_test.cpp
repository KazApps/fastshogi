...
TEST_SUITE("Option Parsing Tests") {
    ...
    TEST_CASE("Config option loads configuration file") {
        const cli::OptionsParser parser{
            buildArgs({"fastshogi.exe", "-config", "file=app/tests/configs/config.json", "stats=false",
                       "outname=custom.json"})};
    ...
    TEST_CASE("Debug option reports helpful error") {
        CHECK_THROWS_WITH_AS(cli::OptionsParser{baseArgs({"-debug"})},
                             "Error while reading option \"-debug\" with value \"-debug\"\nReason: The 'debug' option does not exist in fastshogi. Use the 'log' option instead to write all engine input and output into a text file.",
                             fastshogi_exception);
    }

    TEST_CASE("Unknown option should throw an error") {
        const auto args = cli::Args{"fastshogi.exe", "-unknown"};
        CHECK_THROWS_AS(cli::OptionsParser{args}, fastshogi_exception);
    }
    ...
    TEST_CASE("Engine proto and args options parse correctly") {
        const cli::OptionsParser parser{baseArgs({"-each", "proto=usi", "args=--debug --fast"})};
        const auto engines = parser.getEngineConfigs();
    ...
    TEST_CASE("Compliance mode throws until implemented") {
        const auto args = cli::Args{"fastshogi.exe", "--compliance", "app/tests/mock/engine/dummy_engine"};
        CHECK_THROWS_AS(cli::OptionsParser{args}, fastshogi_exception);
    }
